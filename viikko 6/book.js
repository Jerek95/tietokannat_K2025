const book = {
    bookArray : [
        ...
        {'id_book' : "7", 'name' : "Make it in Clay", 'author' : "Stokstad", 'isbn' : "205795617"},
        {'id_book' : "8", 'name' : "Art History Vol II & ala carte lab", 'author' : "Stokstad", 'isbn' : "1111287856"},
        {'id_book' : "9", 'name' : "Accounting Concepts", 'author' : "Albrecht", 'isbn' : "0538479736"},
    ...
    getOneBook: function(x){
        for (i = 0; i < this.bookArray.length; i++){
            if (this.bookArray[i]['name'] == x){
                console.log(this.bookArray[i]);
            }
        }
    },
    addBook: function(id_book, name, author, isbn){
        this.bookArray.push({'id_book':id_book, 'name':name, 'author':author, 'isbn':isbn});
    }
};

book.getAllBooks();
book.getOneBook("Leadership Wisdom of Jesus");
book.addBook("16", "Raamattu", "Jumala", "1234567890");
book.getAllBooks();